...
// 2. 在 cmd_price 函数中添加 market 变量（在 name 之后）
const oldNameLine = 'local name=$(get_etf_name "$code")\n    echo -e "${GREEN}📈 $name ($code) 实时行情${NC}"';
const newNameLine = 'local name=$(get_etf_name "$code")\n    # 确定市场前缀\n    local market="sh"\n    if [[ "$code" == 15* ]] || [[ "$code" == 0* ]] || [[ "$code" == 3* ]]; then\n        market="sz"\n    fi\n    echo -e "${GREEN}📈 $name ($code) 实时行情${NC}"';
...
// 5. 替换数据解析逻辑 - 使用简化的腾讯 API 解析
const oldPython1 = 'local current=$(echo "$response" | python3 -c "\nimport json, sys\ntry:\n    data = json.load(sys.stdin)\n    result = data.get(\'chart\', {}).get(\'result\', [{}])[0]\n    meta = result.get(\'meta\', {})\n    print(meta.get(\'regularMarketPrice\', \'N/A\'))\nexcept: print(\'N/A\')\n" 2>/dev/null)';
const newCurrent = 'local data=$(echo "$response" | sed "s/v_${market}${code}=\\"//" | sed \'s/"$//\')\n        local current=$(echo "$data" | cut -d\'~\' -f4)';
content = content.replace(oldPython1, newCurrent);

const oldPython2 = 'local prev_close=$(echo "$response" | python3 -c "\nimport json, sys\ntry:\n    data = json.load(sys.stdin)\n    result = data.get(\'chart\', {}).get(\'result\', [{}])[0]\n    meta = result.get(\'meta\', {})\n    print(meta.get(\'previousClose\', \'N/A\'))\nexcept: print(\'N/A\')\n" 2>/dev/null)';
const newPrevClose = 'local prev_close=$(echo "$data" | cut -d\'~\' -f5)';
content = content.replace(oldPython2, newPrevClose);

const oldPython3 = 'local change=$(echo "$response" | python3 -c "\nimport json, sys\ntry:\n    data = json.load(sys.stdin)\n    result = data.get(\'chart\', {}).get(\'result\', [{}])[0]\n    meta = result.get(\'meta\', {})\n    diff = float(meta.get(\'regularMarketPrice\', 0)) - float(meta.get(\'previousClose\', 0))\n    pct = diff / float(meta.get(\'previousClose\', 1)) * 100\n    print(f\'{diff:+.4f} ({pct:+.2f}%)\')\nexcept: print(\'N/A\')\n" 2>/dev/null)';
const newChangePct = 'local change=$(echo "$data" | cut -d\'~\' -f32)\n        local pct=$(echo "$data" | cut -d\'~\' -f31)';
content = content.replace(oldPython3, newChangePct);

// 6. 替换输出部分
const oldOutput = 'echo -e "当前价格：${GREEN}$current${NC}"\n        echo -e "昨收：$prev_close"\n        echo -e "涨跌：$change"\n        \n        # 获取涨跌幅颜色\n        if [[ "$change" == +* ]]; then\n            echo -e "${GREEN}📈 上涨${NC}"\n        else\n            echo -e "${RED}📉 下跌${NC}"\n        fi';
const newOutput = 'local color="${NC}"\n        local arrow="➡️"\n        local trend="平盘"\n        if [ -n "$change" ] && [ "$change" != "0.000" ] && [ "$change" != "0" ]; then\n            local is_pos=$(echo "$change > 0" | bc -l 2>/dev/null)\n            local is_neg=$(echo "$change < 0" | bc -l 2>/dev/null)\n            if [ "$is_pos" = "1" ]; then\n                color="${GREEN}"; arrow="📈"; trend="上涨"\n            elif [ "$is_neg" = "1" ]; then\n                color="${RED}"; arrow="📉"; trend="下跌"\n            fi\n        fi\n        echo -e "当前价格：${color}¥${current}${NC}"\n        echo -e "昨收：¥${prev_close}"\n        echo -e "涨跌：${color}${change} (${pct}%)${NC}"\n        echo -e "${color}${arrow} ${trend}${NC}"';
