...
cmd_price() {
    ...
    # 使用curl获取数据
    local response=$(curl -s "http://qt.gtimg.cn/q=${market}${code}" 2>/dev/null | iconv -f gbk -t utf-8 2>/dev/null | head -100)
    ...
    if echo "$response" | grep -q "v_${market}${code}="; then
        ...
        local change=$(echo "$data" | cut -d'~' -f32)
        local pct=$(echo "$data" | cut -d'~' -f33)
        
        echo -e "当前价格: ${GREEN}$current${NC}"
        echo -e "昨收: $prev_close"
        echo -e "涨跌: $change"
        
        # 判断涨跌方向（腾讯 API 返回的 change 不带符号）
        local color="${NC}"
        local arrow="➡️"
        local trend="平盘"
...
cmd_compare() {
    ...
    # 获取两只ETF的行情
    local price1=$(curl -s "http://qt.gtimg.cn/q=${code1}.SS" 2>/dev/null | python3 -c "
import json, sys
try:
...
" 2>/dev/null)
    ...
    local price2=$(curl -s "http://qt.gtimg.cn/q=${code2}.SS" 2>/dev/null | python3 -c "
import json, sys
try:
    data = json.load(sys.stdin)
    result = data.get('chart', {}).get('result', [{}])[0]
