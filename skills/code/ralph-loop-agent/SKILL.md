...
- Max iterations (default: PLANNING=3, BUILDING=6; hard cap: PLANNING<=5, BUILDING<=8)

**Optional**:
- Completion sentinel (default: `STATUS: COMPLETE` in `IMPLEMENTATION_PLAN.md`)
- Working directory (default: `$PWD`)
- Timeout per iteration (default: 900s; hard cap: 1800s)
- Sandbox choice
...
3. **Set timeouts**: Default 900s per iteration (hard cap 1800s); if timeout happens 2 consecutive rounds, stop loop with `STATUS: LOOP_ABORT_TIMEOUT`
