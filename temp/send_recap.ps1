...
$enc = [System.Text.Encoding]::UTF8.GetBytes($body)
$req = [System.Net.HttpWebRequest]::CreateHttp("https://api.telegram.org/bot" + $token + "/sendMessage")
$req.Method = "POST"
$req.ContentType = "application/json; charset=utf-8"
$req.ContentLength = $enc.Length
$req.UserAgent = "OpenClaw/1.0"
$st = $req.GetRequestStream()
$st.Write($enc, 0, $enc.Length)
$st.Close()
try {
    $resp = $req.GetResponse()
    $sr = New-Object System.IO.StreamReader($resp.GetResponseStream())
    Write-Host "OK:" $sr.ReadToEnd()
    $sr.Close()
} catch {
